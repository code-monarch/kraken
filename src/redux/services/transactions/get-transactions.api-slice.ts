    ...
    getTransactions: builder.query<ITransactionsResponse, ITransactionsPayload>(
      {
        query: ({ pageSize, page, searchQuery, filterby, type }) => ({
          url: `dev/admin/transactions/?page=${page}&limit=${pageSize}${filterby ? `&filterby=${filterby.label};${filterby.value ?? "COMPLETED"},type;${type ?? "Deposit"},` : ""}${searchQuery ? `&searchQuery=${searchQuery}` : ""}`,
          method: 'GET',
          headers: {
